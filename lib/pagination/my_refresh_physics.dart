...
/// in [ClampingScrollPhysics], it doesn't allow to flip out of edge,but in MyRefreshPhysics,it will allow to do that,
...
class MyRefreshPhysics extends ScrollPhysics {
  ...
  @override
  MyRefreshPhysics applyTo(ScrollPhysics? ancestor) {
    return MyRefreshPhysics(
  ...
  double applyPhysicsToUserOffset(ScrollMetrics position, double offset) {
    ...
    if (controller!.headerMode!.value == RefreshStatus.twoLeveling) {
      ...
      if ((offset > 0.0 &&
              viewportRender?.firstChild is! MyRenderSliverRefresh) ||
          (offset < 0 && viewportRender?.lastChild is! MyRenderSliverLoading)) {
        return parent!.applyPhysicsToUserOffset(position, offset);
      }
    }
  ...
  double applyBoundaryConditions(ScrollMetrics position, double value) {
    ...
    final bool enablePullUp = viewportRender == null
        ? false
        : viewportRender!.lastChild is MyRenderSliverLoading;
    if (controller!.headerMode!.value == RefreshStatus.twoLeveling) {
    ...
    if (enablePullDown) {
      final MyRenderSliverRefresh sliverHeader =
          viewportRender!.firstChild as MyRenderSliverRefresh;
      topExtra = sliverHeader.hasLayoutExtent
          ? 0.0
          : sliverHeader.refreshIndicatorLayoutExtent;
    }
    if (enablePullUp) {
      final MyRenderSliverLoading? sliverFooter =
          viewportRender!.lastChild as MyRenderSliverLoading?;
