...
typedef RefresherBuilder = Widget Function(BuildContext context, MyRefreshPhysics physics);
...
/// [RefreshController] must not be null,Only one controller to one MyRefresher
///
/// header,I have finished a lot indicators,you can checkout [MyClassicHeader],[WaterDropMaterialHeader],[MaterialMyClassicHeader],[WaterDropHeader],[BezierCircleHeader]
/// footer,[MyClassicFooter]
///If you need to custom header or footer,You should check out [CustomHeader] or [CustomFooter]
///
/// See also:
///
/// * [RefreshConfiguration], A global configuration for all MyRefresher in subtrees
///
...
class MyRefresher extends StatefulWidget {
  ...
  static MyRefresher? of(BuildContext? context) {
    return context!.findAncestorWidgetOfExactType<MyRefresher>();
  }

  static MyRefresherState? ofState(BuildContext? context) {
    return context!.findAncestorStateOfType<MyRefresherState>();
  }

  @override
  State<StatefulWidget> createState() {
    // TODO: implement createState
    return MyRefresherState();
  }
}

class MyRefresherState extends State<MyRefresher> {
  MyRefreshPhysics? _physics;
  bool _updatePhysics = false;
  double viewportExtent = 0;
  bool _canDrag = true;

  final RefreshIndicator defaultHeader = const MyClassicHeader();

  final LoadIndicator defaultFooter = const MyClassicFooter();
  ...
  ScrollPhysics _getScrollPhysics(
    ...
    return _physics = MyRefreshPhysics(
  ...
  @override
  void didUpdateWidget(MyRefresher oldWidget) {
...
class MyRefreshController {
  ...
  /// initialRefresh:When MyRefresher is init,it will call requestRefresh at once
  ///
  ...
  void _bindState(MyRefresherState state) {
    assert(_refresherState == null,
        "Don't use one refreshController to multiple MyRefresher,It will cause some unexpected bugs mostly in TabBarView");
...
/// Controls how MyRefresher widgets behave in a subtree.the usage just like [ScrollConfiguration]
///
/// The refresh configuration determines MyRefresher some behaviours,global setting default indicator
///
/// see also:
///
/// * [MyRefresher], a widget help attach the refresh and load more function
class RefreshConfiguration extends InheritedWidget {
  ...
  ///
  /// it mostly use in some stiuation is different the other MyRefresher in App
